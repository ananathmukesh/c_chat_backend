...
try {
  ...
  (SELECT TOP 1 time FROM message m WHERE (m.sender_id=${id} and m.receiver_id=users.id) or (m.sender_id=users.id and m.receiver_id=${id}) ORDER BY message DESC) as time,
  ...
  (SELECT TOP 1 unread FROM message m WHERE (m.sender_id=${id} and m.receiver_id=users.id) or (m.sender_id=users.id and m.receiver_id=${id}) ORDER BY message DESC) as unread,
