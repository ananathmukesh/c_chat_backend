...
try {
  ...
  (SELECT TOP 1 time FROM message m WHERE (ch.sender_id=${id} and ch.receiver_id=users.id) or (ch.sender_id=users.id and ch.receiver_id=${id}) ORDER BY message DESC) as time,
  (SELECT TOP 1 message FROM message m WHERE (ch.sender_id=${id} and ch.receiver_id=users.id) or (ch.sender_id=users.id and ch.receiver_id=${id}) ORDER BY id DESC) as msg,
  (SELECT TOP 1 unread FROM message m WHERE (ch.sender_id=${id} and ch.receiver_id=users.id) or (ch.sender_id=users.id and ch.receiver_id=${id}) ORDER BY message DESC) as unread,
